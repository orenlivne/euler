...
class Friends:
    ...
    def add(self, connection):
        if self.g.has_edge(*connection): return False
        self.g.add_edge(*connection)
        return True

    def remove(self, connection):
        if not self.g.has_edge(*connection): return False
        self.g.remove_edge(*connection)
        for u in connection:
            if self.g.degree(u) == 0: self.g.remove_node(u)
        return True
    ...
    def connected(self, name):
        return set(self.g.neighbors(name) if name in self.g else [])
