...
"Building({south}, {west}, {width[1]}, {width[0]}, {height})"
...
class Building:
    def __init__(self, south, west, width_WE, width_NS, height=10):
        self.sw = (south, west)
        self.width = (width_NS, width_WE)
        self.height = height

    def corners(self):
        return {'north-east': [self.sw[0] + self.width[0], self.sw[1] + self.width[1]],
                'south-east': [self.sw[0], self.sw[1] + self.width[1]],
                'south-west': [self.sw[0], self.sw[1]],
                'north-west': [self.sw[0] + self.width[0], self.sw[1]],}
    ...
    def __repr__(self):
        return 'Building(%s, %s, %s, %s, %s)' % tuple(map(str, (self.sw[0], self.sw[1], self.width[1], self.width[0], self.height)))
