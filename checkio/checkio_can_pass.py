...
def shortest_path_distance(g, source):
  ...
  while Q:
    ...
    u, dist_u = Q.pop()
    print 'u', u, 'dist_u', dist_u, 'neighbors', g.nbhrs[u].keys()
    for v, weight in g.nbhrs[u].iteritems():  # where v is still in Q.
      alt = dist_u + weight
      if alt < dist[v]:               # A shorter path to v has been found.
        print 'Updating distance of v', v, 'from', dist[v], 'to', alt
        dist[v] = alt
        Q.add(v, alt)
        prev[v] = u
  return dist, prev
...
def can_pass(a, first, second):
  ...
  dist, prev = shortest_path_distance(g, first)
  print dist
