...
def optimal_alignment(x, y, score, gap, debug=False):
    '''Optimal alignment that maximizes the alignment score based on the (symmetric) ScoringMatrix score
    and a gap penalty \'gap\'.
    
    Integrated DP+backtracking, O(mn) time, O(min(m,n)) storage.'''
    m, n = len(x), len(y)
    if m < n:
        d, (yy, xx) = optimal_alignment(y, x, score, gap, debug=debug)
        return d, (xx, yy)
    c, c_old, a, a_old = range(0, -gap * (n + 1), -gap), [0] * (n + 1), [tuple([S_GAP] * i) for i in xrange(n + 1)], [()] * (n + 1)
    if debug: print c
    for i, xi in enumerate(x, 1):
        c_old[:] = c[:]; a_old[:] = a[:]
        # Initial condition
        c[0] = -gap * i; a[0] = a_old [0 ] + (T_GAP,)
        for j, yj in enumerate(y, 1):
            if xi == yj: c[j], a[j] = c_old[j - 1] - score(xi, yj), a_old[j - 1] + (EQ,)
            else:
                # Possible paths to extend to (i,j); pick best one (greedy approach)
                c1, c2, c3 = c_old[j - 1] - score(xi, yj), c_old[j] - gap, c[j - 1] - gap
                c_min = min(c1, c2, c3)
                if c_min == c1: a_min = a_old[j - 1] + (EQ,)
                elif c_min == c2: a_min = a_old[j] + (T_GAP,)
...
if __name__ == "__main__":
    d, (xx, yy) = optimal_alignment('PLEASANTLY', 'MEANLY', ra.BLOSUM62, -5, debug=True)
    print d
    print xx
    print yy
