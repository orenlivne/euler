...
cdef DTYPE_t MINUS_INFINITY = -sys.maxint - 1 + 10000
...
def optimal_alignment_array_form(char * x, char * y, int gap_init, int gap_ext, int debug, char * gap_symbol):
    ...
    cdef DTYPE_t RESTART = 0  # if local else MINUS_INFINITY
    cdef int m = len(x)
    cdef int n = len(y)
    cdef int i
    cdef int j
    if debug >= 1:
        print m, n
        start = time.time()
    cdef np.ndarray c = np.zeros([m + 1, n + 1], dtype=DTYPE)  # Max score of paths ending at (i,j)
    cdef np.ndarray d = np.zeros([m + 1, n + 1], dtype=DTYPE)  # Max score of paths ending at (i,j), ending with a substitution
    cdef np.ndarray e = np.zeros([m + 1, n + 1], dtype=DTYPE)  # Max score of paths ending at (i,j), ending with an ins/del
