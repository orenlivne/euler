...
See Figure 1 for a graph of the common logarithm function y=log10(x). In this graph, we can see that the logarithm of x-values between 0 and 1 always winds up mapping to y-values between -INFTY and 0: x-values near 0 have logarithms close to -INFTY, and x-values close to 1 have logarithms close to 0. Thus, we will select the common logarithm as our function to "blow up" small probability values for comparison.
...
if __name__ == "__main__":
    print prob('rosalind_prob_sample.dat')
    print prob('rosalind_prob.dat')
